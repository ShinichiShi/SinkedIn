...
export default function CategoryPage({ params }: { params: { id: string } }) {
  ...
  // Use refs to track current state in scroll handler
  const loadingMoreRef = useRef(loadingMore);
  const hasMoreRef = useRef(hasMore);
  const fetchCategoryPostsRef = useRef<((isLoadMore?: boolean) => Promise<void>) | null>(null);
  ...
  // Store the function in ref for scroll handler
  useEffect(() => {
    fetchCategoryPostsRef.current = fetchCategoryPosts;
  }, [fetchCategoryPosts]);
  ...
  const loadMorePosts = useCallback(() => {
    if (!loadingMoreRef.current && hasMoreRef.current && fetchCategoryPostsRef.current) {
      console.log('Loading more posts...');
      fetchCategoryPostsRef.current(true);
    }
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Trigger when user is near bottom (100px before reaching end)
      if (scrollTop + clientHeight >= scrollHeight - 100) {
        console.log('Near bottom, checking if can load more...');
        console.log('Loading more:', loadingMoreRef.current, 'Has more:', hasMoreRef.current);
        loadMorePosts();
      }
    };
